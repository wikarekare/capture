...
def ip_address_to_subnet(ipaddr)
  ...
  elsif ip_dot[0] == '100' && ip_dot[1] == '64' #100.64 addresses we only need to record to /27
    IPAddr.new(ipaddr + '/27').to_s
...
def mac_address(last_bytes)
  case last_bytes
  when '0EF8'; "E0:46:9A:0B:0E:F8" #ADSL1
  when '1270'; "E0:46:9A:0B:12:70" #ADSL2
  when '0B2C'; "E0:46:9A:0B:0B:2C" #ADSL3
  when '97A4'; "C4:3D:C7:BD:97:A4" #ADSL4
  when DSL5_SHORT_MAC ; DSL5_MAC #ADLS5 VDSL1
  when DSL6_SHORT_MAC ; DSL6_MAC #ADLS6 VDSL2
  when DSL7_SHORT_MAC ; DSL7_MAC #ADLS7 VDSL3
  when GATE_SHORT_MAC ; GATE_MAC #Admin1 ADSL net interface
...
def parse_line(line)
  ...
  begin
    ...
    puts "add_record returned #{error}"
  end
end
...
=end
    ...
    STDERR.puts "put_records:" + error.to_s
  ensure 
    fd.close if fd != nil
  end
end
...
def log_name_to_date_interval(filename)
  ...
  begin
    ...
    puts "log_name_to_date_interval(#{filename}): #{error}"
    return nil,nil
  end
end
